...
專案 Conda 環境設定範本 - 供 Claude Code 自動化執行
...
# 常用套件
conda install -y pip numpy pandas matplotlib requests
...
```bash
# 激活環境
source ~/.zshrc && conda activate ${PROJECT_NAME}

# 驗證環境
echo "當前環境：$CONDA_PREFIX"
...
```bash
# 在正確環境中執行 Python
source ~/.zshrc && conda activate ${PROJECT_NAME} && python script.py
```
...
```bash
# 使用完整路徑
/opt/homebrew/anaconda3/bin/conda activate ${PROJECT_NAME}
```
...
1. **複製此檔案到新專案資料夾**
2. **調整 Python 版本**：根據需求設定 `PYTHON_VERSION`（專案名稱會自動取得）
3. **執行環境建立**：讓 Claude Code 執行「環境建立與設定」區塊
4. **驗證環境**：確認環境正確建立

### Claude Code 使用方式：

1. **讀取此檔案**：`請閱讀 CLAUDE_CONDA.md 並設定專案環境`
2. **執行設定**：Claude Code 會自動執行環境建立指令
3. **驗證環境**：確認環境正確激活
4. **開始開發**：在正確環境中執行後續指令
...
```bash
# 在正確環境中啟動開發伺服器
source ~/.zshrc && conda activate ${PROJECT_NAME} && python app.py
```

## 重要提醒

⚠️ **Claude Code 執行前必須先激活環境**
```bash
source ~/.zshrc && conda activate ${PROJECT_NAME}
```
...
# 確認在正確環境中
which pip
# 應該顯示：/opt/homebrew/anaconda3/envs/${PROJECT_NAME}/bin/pip
```

---

**範本版本：** v1.0  
**適用系統：** macOS (Homebrew Anaconda)  
**最後更新：** 2025-07-09